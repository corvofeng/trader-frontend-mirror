  ...
  updateWhitelist: async (id: string | number, whitelist: Partial<OptionWhitelist>, userId: string, accountId?: string | null): Promise<ServiceResponse<OptionWhitelist>> => {
    ...
    return {
      data: {
        ...
        contract_code_full: whitelist.contract_code_full,
    ...
    const base: SequentialTradeTask[] = [
      {
        ...
        env: 'prod',
        combo_id: 'COMBO-001',
        expiry_date: toIso(new Date(now.getTime() + 7 * 24 * 60 * 60 * 1000)),
      ...
      },
      {
        ...
        env: 'staging',
        combo_id: 'COMBO-002',
        expiry_date: toIso(new Date(now.getTime() + 14 * 24 * 60 * 60 * 1000)),
